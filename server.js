...
class StatisticData {
     ...
     * @param {number} hpLessenValue - HP reduction amount (only for damage)
     */
...
class UserData {
    constructor(uid) {
        ...
        this.skillUsage = new Map(); // Skill usage
        this.fightPoint = 0; // Total rating
        this.subProfession = '';
        this.attr = {};
    }
...
class UserDataManager {
    constructor(logger) {
        ...
        this.hpCache = new Map(); // HP that changes frequently is not saved to disk
    ...
    /** Initialize method - asynchronously load user cache */
    async initialize() {
    ...
    /** Force immediate save user cache - for program exit scenarios */
     ...
     * @param {number} damage - Damage taken value
     ...
     * @param {number} uid - User ID
     * @param {number} fightPoint - Total rating
     */
...
async function main() {
    ...
    // If command line not provided or invalid, use interactive mode
    ...
    // Save user cache on process exit
    ...
    // Download historical battle log data
    ...
    const processEthPacket = async (frameBuffer) => {
        ...
        if (current_server !== src_server && current_server !== src_server_re) {
            ...
            try {
                //Try to identify server through a reporting small packet
                if (buf[4] == 0 && buf[5] == 5) {
                    const data = buf.subarray(10);
                    if (data.length) {
                        const stream = Readable.from(data, { objectMode: false });
    ...
    //Regularly clean up expired IP fragment cache
    setInterval(async () => {
        const now = Date.now();
...
if (!zlib.zstdDecompressSync) {
    // Previously people always used old versions of Node.js, ignored warnings and complained about inaccurate data, now we simply don't allow old versions to be used
    // Some people write closed-source code based on open-source code, don't follow the license and even slander open-source, what kind of people are these
